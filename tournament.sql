...
create TABLE matches(winner INTEGER, loser INTEGER, match_id SERIAL Primary KEY);
-- first check if Matches exists, if so, it will be dropped. If not: CREATE TABLE
create VIEW number_of_wins AS select players.player_id, players.name, count(matches.winner) AS wins FROM players, matches where players.player_id = matches.winner group by players.player_id;
...
create VIEW standings AS SELECT matches_played.player_id, matches_played.name, COALESCE(number_of_wins.wins,0) As wins, matches_played.matches from matches_played left join number_of_wins on matches_played.player_id = number_of_wins.player_id OR wins = 0 Order By wins desc;